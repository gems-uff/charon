...
public class KnowledgeBase {
	...
	private void setCharonRules(boolean IsUsingDatabase) {
		...
		if(IsUsingDatabase){
			...
			// EXPERIMENT
			charonRules.add("(create_experiment(ExperimentId, ExperimentName, ExperimentRootProcessId) :- assertz(experiment(ExperimentId)), nextVersionId(VersionId), assertz(version(experiment(ExperimentId), VersionId, '0')), assertz(currentVersion(experiment(ExperimentId), VersionId)), assertz(experimentName(VersionId, ExperimentName)), assertz(experimentRootProcessId(VersionId, ExperimentRootProcessId)))");
			charonRules.add("(update_experiment(ExperimentId, ExperimentName, ExperimentRootProcessId) :- nextVersionId(VersionId), currentVersion(experiment(ExperimentId), PreviousVersion), assertz(version(experiment(ExperimentId), VersionId, PreviousVersion)), assertz(experimentName(VersionId, ExperimentName)), assertz(experimentRootProcessId(VersionId, ExperimentRootProcessId)))");
			charonRules.add("(update_experimentName(ExperimentId, ExperimentName) :- currentVersion(experiment(ExperimentId), currentVersion), experimentRootProcessId(VersionId, ExperimentRootProcessId), update_experiment(ExperimentId, ExperimentName, ExperimentRootProcessId))");
			charonRules.add("(update_experimentRootProcess(ExperimentId, ExperimentRootProcessId) :- currentVersion(experiment(ExperimentId), currentVersion), experimentName(VersionId, ExperimentName), update_experiment(ExperimentId, ExperimentName, ExperimentRootProcessId))");
			...
			// ACTIVITY
			charonRules.add("(create_activity(ActivityClassId, ActivityClassName, ActivityClassType, ActivityClassPorts) :- assertz(activity(ActivityClassId)), nextVersionId(VersionId), assertz(version(activity(ActivityClassId), VersionId, '0')), assertz(currentVersion(activity(ActivityClassId), VersionId)), assertz(activityType(VersionId, ActivityClassType)), assertz(activityName(VersionId, ActivityClassName)), associate_activityPort(VersionId, ActivityClassPorts))");
			...
			charonRules.add("(update_activityName(ActivityClassId, ActivityClassName) :- currentVersion(activity(ActivityClassId), currentVersion), activityType(currentVersion, ActivityClassType), findall(E, activityPort(currentVersion, E), Es), update_activity(ActivityClassId, ActivityClassName, ActivityClassType, Es))");
			charonRules.add("(update_activityType(ActivityClassId, ActivityClassType) :- currentVersion(activity(ActivityClassId), currentVersion), activityName(currentVersion, ActivityClassName), findall(E, activityPort(currentVersion, E), Es), update_activity(ActivityClassId, ActivityClassName, ActivityClassType, Es))");
			charonRules.add("(add_activityPort(ActivityClassId, PortId) :- currentVersion(activity(ActivityClassId), currentVersion), activityName(currentVersion, ActivityClassName), activityType(currentVersion, ActivityClassType), findall(E, activityPort(currentVersion, E), Es), update_activity(ActivityClassId, ActivityClassName, ActivityClassType, Es))");
			charonRules.add("(remove_activityPort(ActivityClassId, PortId) :- currentVersion(activity(ActivityClassId), currentVersion), activityName(currentVersion, ActivityClassName), activityType(currentVersion, ActivityClassType), retract(activityPort(currentVersion, PortId)), findall(E, activityPort(currentVersion, E), Es), assertz(activityPort(currentVersion, PortId)), update_activity(ActivityClassId, ActivityClassName, ActivityClassType, Es))");
			charonRules.add("(associate_activityPort(ActivityClassVersionId, []))");
			charonRules.add("(associate_activityPort(ActivityClassVersionId, [ActivityClassPort|ActivityClassPorts]) :- assertz(activityPort(ActivityClassVersionId, ActivityClassPort)), associate_activityPort(ActivityClassVersionId, ActivityClassPorts))");
			...
			charonRules.add("(update_SWFMSName(SWFMS_Id, SWFMS_Name) :- currentVersion(swfms(SWFMS_Id), currentVersion), swfmsHost(currentVersion, SWFMS_Host), update_SWFMS(SWFMS_Id, SWFMS_Name, SWFMS_Host))");
			charonRules.add("(update_SWFMSHost(SWFMS_Id, SWFMS_Host) :- currentVersion(swfms(SWFMS_Id), currentVersion), swfmsName(currentVersion, SWFMS_Name), update_SWFMS(SWFMS_Id, SWFMS_Name, SWFMS_Host))");
